						...
						<tr>
							...
							<td class="text-center">
								<div  class="btn btn-danger" style="margin-left: 20px;">
									<a href="order.php?action=remove-all&maphieu=<?=$maphieu?>&magiuong=<?=$magiuong?>"><i class="fa fa-times-circle" style="color:#fff"></i></a>
								</div>
							</td>
						</tr>
...
if (isset($_SESSION['MaHangBan']) ) {
	...
	if ($action=="update") {
		...
		( $mahangban_arr=$_POST['mahangban_arr'] );echo count($_POST['mahangban_arr']);
...
having sum(SoLuong) > 0
		...
		while ($r=sqlsrv_fetch_array($rs)){
			...
			else $_SESSION['SoLuong'][$mahangban]=$r['SoLuong'];
...
/*end of this is to avoid empty arra element*/
	...
	{ ?>
		...
		<tr>
			...
			<td class="text-center"><?=number_format($tien,0,",",".")?><sup>đ</sup></td>
			...
			?>
		<tr>
			<td class="text-center"><input type="checkbox" name="id_arr[]" value="<?=$mahangban?>" /></td>
			<td class="col-md-12">
				<div class="media">
					 <!--<a class="thumbnail pull-left" href="#"> <img class="media-object" src="http://lorempixel.com/460/250/" style="width: 72px; height: 72px;"> </a>-->
					 <div class="media-body">
			...
			<td class="text-right">
				<a href="order.php?action=remove&mahangban_xoa=<?=$mahangban?>&mahangban=<?=@$_GET['mahangban']!=""?@$_GET['mahangban']:@$_POST['mahangban']; ?>&pageno=<?=$pageno?>&maphieu=<?=$maphieu?>&magiuong=<?=$magiuong;?>">
					<button type="button" class="btn btn-secondary" id="single-removal" style="background-color:orange"><i class="fa fa-trash-o" style="color:#fff;"></i></button>
...
?>
		...
		<td class="text-center"><?=@$tongsoluong>0?$tongsoluong:"0";?></td>
		
		<td class="text-center"><?=@$tongtien>0?$tongtien:"0";?><sup>đ</sup></td><?php $_SESSION['tongtien'] = $tongtien; ?>
		<input type="hidden" name ="mahangban" value="<?=$mahangban?>" style="width:107px;" />
		<td class="text-right"><input type="hidden" name="magiuong" value="<?=$magiuong?>" />	
